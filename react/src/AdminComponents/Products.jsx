...
function Products() {
  ...
  useEffect(() => {
    axios
      .get(
        `http://127.0.0.1:8000/api/allProducts/${size}/${price}/${stock}/${categorySelect}/${collectionSelect}?page=${page}`
      )
      .then((response) => {
  ...
  function handleSize(id) {
    navigate(`/adminPannel/productSize/${id}`);
  }
  return (
    <>
      {products ? (
