...
function Category() {
  ...
  useEffect(() => {
    axios
      .get(`http://127.0.0.1:8000/api/search/${param.key}/${size}/${price}/${stock}?page=${page}`)
      .then((response) => {
        setLoading(false)
  ...
  }, [param.key, size, price,stock, page]);
  ...
  return (
    <>
                ...
                <div className={styles.product} key={product.id}>
                  ...
                  <ul>
                    {product.sizes.map((size, index) => (
                      <li key={index}>{size.name_Size}</li>
                    ))}
                  </ul>
                </div>
              ))
      ...
      </Stack>:<h1 style={{display:'flex',justifyContent:'center'}}>Product not found</h1>}
    </>
  );
}
